...
class SecondToDoTableViewController: UITableViewController {
    ...
    override func  viewWillAppear(_ animated: Bool) {
        ...
        do {
            SecondToDoList = try context.fetch(fenchRequest)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let text = SecondToDoList[indexPath.row]
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        let item = SecondToDoList[indexPath.row]
        if editingStyle == .delete {
            
            remove2Item(at: indexPath.row, item: item)
